# KothaSetu - Final Patch (removes all template literals)
      ...
      {(mode === 'voice-voice' || mode === 'voice-text') && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: 14 }}>
            {'isListening' && isListening ? 'Listening in ' + fromLangData.name + '... tap to stop' : 'Tap to speak in ' + fromLangData.name}
          </p>
        </div>
      )}
    </div>
...
Write-Host "Patching src\components\translation\Waveform.jsx" -ForegroundColor Cyan
@'
...
Write-Host "Patching src\components\translation\LanguageSelector.jsx" -ForegroundColor Cyan
@'
...
Write-Host "Patching src\hooks\useSpeechRecognition.js" -ForegroundColor Cyan
@'
...
Write-Host "Patching src\services\translation.js" -ForegroundColor Cyan
@'
...
Write-Host "All fixed! Run: npm run dev" -ForegroundColor Green